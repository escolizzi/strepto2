/*
strepto_noB.c: no break points, extra deletions at the end of the chromosome
  ...
  */
  ...
  for(i=1;i<=nrow;i++)for(j=1;j<=ncol;j++)
  {
    ...
    if( genrand_real1()<0.01) // i==nrow/2 && j==nrow/2)
    {
      ...
      //  if(genrand_real2() < 0.05) world[i][j].seq[k] = AZ[2]; //give break points only once in a while  
      }
  ...
  if(world[row][col].val==0){
    ...
    for(k=1;k<9;k++){
      ...
      if(nei->val!=0){
        ...
        double growth=fg/(fg+fgscale);// was -> /(ratio+rscale));
...
void Update(void)
{
  ...
  int proposed_season_change=300;
...
int Mutate(TYPE2** world, int row, int col)
{
  ...
  for(ipos=0;ipos<genome_size;ipos++){
    tot_mutrate = 0.25*(epsilon + (1.-epsilon)*( 1. - exp(-0.0001*pow(ipos,2.)) ));
...
void UpdateABproduction(int row, int col){
  ...
  double agprod=ag/(ag+agscale)*(exp(-3.*fg)); // was -> /(ratio+rscale));
